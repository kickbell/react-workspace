...
function PostModifyCom({ title, content, loading, error, onChange, onSubmit, postId }) {
  return (
    <div style={{ padding: "20px", maxWidth: "900px", margin: "0 auto" }}>
      <h2 style={{ marginBottom: "16px" }}>게시글 수정</h2>
      <form onSubmit={onSubmit} style={{ display: "grid", gap: "12px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px" }}>
          <Link to={`/post/detail/${postId}`} style={postButtonSecondaryStyle}>취소</Link>
          <button
            type="submit"
            disabled={loading}
            style={{
